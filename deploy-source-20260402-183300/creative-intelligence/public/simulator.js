(function() {
            ...
            '<div class="ci-panel">' +
                '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:18px;flex-wrap:wrap;">' +
                    '<div>' +
                        ...
                        '<p style="font-size:13px;color:var(--text-dim);max-width:760px;">Actual portal ROAS vs frozen checkpoint forecasts for immature ad cohorts. Each P0, P2, P8, and P14 line is created from the actual base available on that checkpoint day and never rewritten.</p>' +
                    '</div>' +
                ...
                '<div style="padding:14px 16px;border:1px solid var(--border);border-radius:12px;background:rgba(99,102,241,0.08);margin-bottom:18px;">' +
                    ...
                    '<div style="font-size:12px;color:var(--text-dim);line-height:1.6;">Actuals reuse the current portal snapshot logic. Checkpoint lines are frozen at creation. If a checkpoint is created after a horizon has already passed, the chart locks that earlier horizon to the actual value instead of pretending it was forecasted.</div>' +
                '</div>' +
        ...
        lines.push({
            ...
            points: (data.actualLine || []).map(function(point) {
                return {
                    ...
                    value: point.value,
                    pointType: 'actual',
                    title: 'Actual ' + point.horizon + ': ' + fmtRoas(point.value)
                };
            })
        });
            ...
            lines.push({
                ...
                points: (run.points || []).map(function(point) {
                    var title = run.checkpoint_code + ' ' + point.horizon + ': ' + fmtRoas(point.value) +
                        ' (' + (point.pointType === 'locked_actual' ? 'locked actual' : 'forecast') + ')';
                    return {
                        horizon: point.horizon,
                        day: point.day,
                        value: point.value,
    ...
    function renderTrendline(container, data) {
        ...
        function xPos(idx) {
            return PAD_L + ((idx + 0.5) / horizonCount) * chartW;
        }
        ...
        (data.horizons || []).forEach(function(horizon, idx) {
            svg += '<text x="' + xPos(idx) + '" y="' + (PAD_T + chartH + 22) + '" text-anchor="middle" fill="var(--text-dim, #aaa)" font-size="11" font-weight="700">' + horizon.label + '</text>';
        });
        ...
        lines.forEach(function(line) {
            ...
            plotted.forEach(function(point) {
                var radius = point.pointType === 'locked_actual' ? 4.5 : 4;
                var fill = point.pointType === 'locked_actual' ? '#ffffff' : line.color;
                var stroke = point.pointType === 'locked_actual' ? line.color : '#121212';
                ...
                data.checkpoints.map(function(run) {
                    ...
                    return '<div style="border:1px solid var(--border);border-top:3px solid ' + meta.color + ';border-radius:12px;padding:12px;background:var(--bg);">' +
                        ...
                        '<div style="font-size:11px;color:var(--text-dim);line-height:1.6;">' +
                            ...
                            'Later checkpoints do not rewrite this line.' +
                        '</div>' +
